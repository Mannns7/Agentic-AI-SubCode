...
if __name__ == "__main__":
    ...
    # Test 10 (NEW): a dict position must be destructured BY KEY, not by a
    # regex over str(dict). {"x": 1, "y": 5} used to stringify to "x1y5",
    # get read as "column X, row 1", and return column 23 on a 10-column
    # map - planning from that bogus origin desyncs every move after it.
    ...
    # Test 11 (NEW): a start that lands inside a wall (wrong indexing from
    # the caller) must be calibrated back onto a real walkable tile, or the
    # agent walks into a wall on its very first step.
    walled_map = [
        ["wall", "wall", "wall"],
        ["wall", "start", "normal"],
    ...
    # Test 12 (NEW): walls labelled with anything other than the exact
    # string "wall" must still be impassable. Previously "brick" fell
    # through to a walkable "normal" tile, so the planner routed straight
    # through it and the game rejected the move mid-route.
    brick_map = [
        ["start", "brick", "normal"],
        ["normal", "brick", "normal"],
        ["normal", "normal", "treasure"],
    ...
    print("OK: all 12 self-checks passed")
